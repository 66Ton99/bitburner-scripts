...
async function crawlNeighbors(ns, script, origin, interval, maxAttempts, verboseTerminal) {
  ...
  for (const target of neighbors) {
    ...
    if (details.modelId === '(The Labyrinth)') try {
      ns.print('INFO: Try stasis!');
      //TODO something to maybe create flag-file to make script run once and manualy do it to unstasis
      let a1 = ns.getServerMaxRam(ns.getHostname()) - ns.getServerUsedRam(ns.getHostname());
      if (a1 >= ns.getScriptRam(STASIS_FILE, ns.getHostname())) {
        ns.exec(STASIS_FILE, ns.getHostname(), { threads: 1, preventDuplicates: true }, true);
      } else ns.print('WARN: No ram!');
    } catch (error) {
      ns.print(`WARN: Cannot stasis : ${formatError(error)}`);
    }
...
function buildCandidates(details) {
  ...
  const Length = String(details.passwordLength ?? "");
  ...
  if (model === "PHP 5.4") return PHP(data.replace(/\D/g, ""));
  if (model === "AccountsManager_4.2") return AccountManager(Length);
  return [];
}
...
function AccountManager(leng) {
  ...
  if (leng === '2') {
    ...
    //throw new Error(`leng: ${leng}. Must be 1 or 2.`);
  }
...
function PHP(str) {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    for (const perm of PHP(remaining)) {
