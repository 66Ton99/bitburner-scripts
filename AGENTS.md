...
- In BN3, do not grind long combat/crime faction reputation such as `Slum Snakes` by default just because `--crime-focus` is active. These factions are not strategic for SF3.3; skip them unless explicitly requested with `--first` or all-faction mode.
...
- `autopilot.js --money-focus` is the default BN3 RAM-bootstrap money mode: suppress timed XP mode and non-money side activities such as faction work, Go, gangs, sleeves, and grafting while home RAM is below the BN3 bootstrap target. Once that target is reached, resume normal purchase/install progression.
...
- Throttle the forced BN8 `The Red Pill` purchase attempts. If Daedalus is joined but TRP is still not purchased/affordable, do not relaunch `faction-manager.js` every autopilot loop.
