...
- In BN3 only, after the first non-NeuroFlux augmentation has been installed, prioritize `Sector-12` for `CashRoot Starter Kit` before gang/crime invite rushing. Once CashRoot is affordable or awaiting install, install early, but run `ascend.js` in spend-all mode so current cash is spent on practical permanent purchases/upgrades before installing.
