...
- `casino.js` - Dispatcher for casino automation. By default it runs `casino-blackjack.js`, which will play blackjack and reload the game if it loses (automated save-scumming). You can also run `casino.js --game roulette` to launch `casino-roulette.js`. Once you win 10b, you cannot enter the casino any more. Great way to boost your progress once you make the initial 200k needed to travel to Aevum and use the casino. For best performance, run `kill-all-scripts.js` before you run this, since other running scripts slow down the game's load time.
...
- `work-for-factions.js` - (Requires SF4) Daemon.js will start a version of this to make sure your "focus" work goes to good use, but often you'll want to run with your own arguments to specify what kind of work you want to be doing, depending on your goals for the current BitNode.
  - This script now handles infiltration travel decisions before launching the one-shot runner, and prefers infiltration targets that finish the remaining faction reputation quickly instead of blindly maximizing payout.
...
- `local-sync-server.js` - A local Node.js helper that starts a WebSocket server for Bitburner's Remote API so you can sync files from a local source directory into the game, with support for filtering by extension, subfolder, or explicit file list.

## Uploading Files Into Bitburner

If you want to push your local files into the game with `local-sync-server.js`:

1. Start the local sync server from your repo root:
   - `node local-sync-server.js --source-root /path/to/bitburner-scripts`
2. In Bitburner open `Options -> Remote API`.
3. Set the hostname and port to match the server output.
   - By default this is `127.0.0.1` and `12525`.
4. Press `Connect`.
5. Wait for the upload to finish in the terminal running `local-sync-server.js`.

Useful options:

- `--server <server>` uploads to a different Bitburner server than `home`.
- `--subfolder <path>` uploads files into a subfolder in Bitburner.
- `--download <file>` limits upload to specific files.
...
- `alias xp="run daemon.js -vx --tail"`
...
- `alias start-tight="run daemon.js --looping-mode --recovery-thread-padding 30 --cycle-timing-delay 2000 --queue-delay 10 --tail --silent-misfires --initial-max-targets 64"`
